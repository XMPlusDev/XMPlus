...
func (c *Controller) addServices(users []*protocol.User, tag string) error {
...
func (c *Controller) removeServices(users []string, tag string) error {
	handler, err := c.ibm.GetHandler(context.Background(), tag)
	if err != nil {
		return fmt.Errorf("no such inbound tag: %s", err)
	}
...
func (c *Controller) UpdateInboundLimiter(tag string, updatedUserList *[]api.ServiceInfo) error {
